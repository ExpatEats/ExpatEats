-- Setup Payment System Tables (Clean version - works in any SQL interface)
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        ALTER TABLE "payments" ADD CONSTRAINT "payments_guide_id_guides_id_fk" FOREIGN KEY ("guide_id") REFERENCES "guides"("id") ON DELETE no action ON UPDATE no action;
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'payments_guide_purchase_id_guide_purchases_id_fk'
    ) THEN
        ALTER TABLE "payments" ADD CONSTRAINT "payments_guide_purchase_id_guide_purchases_id_fk" FOREIGN KEY ("guide_purchase_id") REFERENCES "guide_purchases"("id") ON DELETE no action ON UPDATE no action;
    END IF;
END $$;
