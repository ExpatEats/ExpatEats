  ...
  try {
    ...
    const enhancedStores: Partial<InsertPlace>[] = [
      // Example entries - replace with your actual Excel data
      {
        ...
        name: "Celeiro - Chiado",
        description: "Portugal's leading health food chain offering organic products, supplements, and natural foods. Extensive gluten-free and vegan selection with knowledgeable staff.",
        address: "Rua do Carmo 88, 1200-092 Lisboa",
        city: "lisbon",
        region: "Greater Lisbon",
        country: "Portugal",
        category: "Health Food Store",
        tags: ["organic", "gluten-free", "vegan", "supplements", "natural-products"],
        latitude: "38.7114",
        longitude: "-9.1394",
      ...
      },
      {
        ...
        description: "Saturday organic market featuring local farmers and producers. Fresh seasonal vegetables, artisanal breads, and traditional Portuguese organic products.",
        address: "Praça do Príncipe Real, 1250-096 Lisboa",
        city: "lisbon",
        region: "Greater Lisbon", 
        country: "Portugal",
        category: "Farmers Market",
        tags: ["organic", "local-farms", "fresh-vegetables", "weekend-market"],
        latitude: "38.7158",
        longitude: "-9.1497",
      ...
      },
      {
        ...
        description: "Major Portuguese supermarket chain with comprehensive Bio section. Wide selection of organic produce, gluten-free products, and international health foods.",
        address: "Av. da República 50, 1050-196 Lisboa",
        city: "lisbon",
        region: "Greater Lisbon",
        country: "Portugal", 
        category: "Supermarket",
        tags: ["organic", "gluten-free", "dairy-free", "bulk-buying", "bio-section"],
        latitude: "38.7372",
        longitude: "-9.1457",
    ...
    for (const store of enhancedStores) {
      try {
        ...
        const existing = store.uniqueId ? 
          await db.select().from(places).where(eq(places.uniqueId, store.uniqueId)).limit(1) :
          await db.select().from(places).where(eq(places.name, store.name!)).limit(1);
    ...
    console.log(`Enhanced store import completed. Imported ${importedCount} new stores.`);
...
export async function importFromExcelData(excelData: any[]) {
  ...
  try {
    ...
    for (const row of excelData) {
      try {
        ...
        const storeData: Partial<InsertPlace> = {
          ...
          freshVegetables: Boolean(row.freshVegetables || row.fresh_vegetables),
          farmRaisedMeat: Boolean(row.farmRaisedMeat || row.farm_raised_meat),
        ...
        const existing = storeData.uniqueId ? 
          await db.query.places.findFirst({
            where: (places, { eq }) => eq(places.uniqueId, storeData.uniqueId)
          }) :
          await db.query.places.findFirst({
            where: (places, { eq }) => eq(places.name, storeData.name!)
          });
    ...
    console.log(`Excel import completed. Imported ${importedCount} new stores.`);
    return { success: true, count: importedCount };
