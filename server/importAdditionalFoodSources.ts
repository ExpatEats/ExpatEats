  ...
  try {
    ...
    const additionalFoodSources: InsertPlace[] = [
      // Farm-Raised Meat Sources
      {
        ...
        description: 'Biomercado offers organic, ethically sourced meat from local and regional farms, including beef, pork, and poultry.',
      ...
      },
      {
        ...
        description: 'Traditional market with butchers offering farm-sourced meats including pasture-raised and locally butchered cuts.',
      ...
      },
      {
        ...
        description: 'Several vendors sell farm-direct meat, including sausages and specialty cuts, often from small Portuguese producers.',
      ...
      {
        name: 'Miosótis Bio',
        description: 'Organic market known for high-quality produce and meats, including traceable farm-raised cuts of beef and poultry.',
        address: 'Av. 5 de Outubro 143D, 1050-052 Lisboa',
        city: 'Lisbon',
        country: 'Portugal',
        category: 'organic supermarket',
        tags: ['farm-raised meat', 'organic', 'traceable', 'non-processed', 'bulk'],
        imageUrl: 'https://via.placeholder.com/150',
        website: 'https://www.miosotis-bio.pt/',
      },
      {
        name: 'Herdade do Freixo do Meio',
        description: 'A renowned organic farm offering direct-to-consumer meat deliveries, specializing in regenerative farming and Iberian breeds.',
        address: 'Online and available at local Lisbon farmer markets',
        city: 'Lisbon',
        country: 'Portugal',
        category: 'organic farm',
        tags: ['farm-raised meat', 'organic', 'regenerative', 'delivery', 'direct purchase'],
        imageUrl: 'https://via.placeholder.com/150',
      ...
      {
        name: 'Go Natural',
        description: 'Focuses on organic and natural products with some locations offering grains, legumes, and nuts in bulk packaging.',
    ...
    const additionalTags = [
      ...
      { name: 'Maria Granel', tags: ['kid-friendly', 'bulk', 'zero-waste'] },
      { name: 'GreenBeans', tags: ['kid-friendly', 'zero-waste'] },
      { name: 'Lidl', tags: ['kid-friendly', 'bulk value packs'] },
      { name: 'Auchan', tags: ['bulk formats'] },
    ];

    console.log(`Importing ${additionalFoodSources.length} additional Lisbon food sources...`);
    ...
    await db.transaction(async (tx) => {
      ...
      for (const tagUpdate of additionalTags) {
        ...
        const [existingPlace] = await db.select().from(places).where(eq(places.name, tagUpdate.name));
