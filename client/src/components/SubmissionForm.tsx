...
export function SubmissionForm({ type, buttonText, buttonClassName }: SubmissionFormProps) {
  ...
  const dialogTitle = type === 'event' ? 'Submit New Event' : 'Submit New Location';
  const dialogDescription = type === 'event' 
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button className={buttonClassName || "bg-accent hover:bg-accent/90 text-white"}>
          <Plus className="h-4 w-4 mr-2" />
          {buttonText || defaultButtonText}
        </Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto" aria-describedby="submission-dialog-description">
        <DialogHeader>
          <DialogTitle id="submission-dialog-title">{dialogTitle}</DialogTitle>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>{type === 'event' ? 'Event Name' : 'Business/Location Name'}</FormLabel>
                  <FormControl>
                    <Input placeholder={type === 'event' ? 'Name of the event' : 'Name of the business or location'} {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="City, address, or area" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
