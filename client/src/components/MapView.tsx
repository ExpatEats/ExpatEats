...
export function MapView({ places, onPlaceClick }: MapViewProps) {
  ...
  const geocodeAddress = async (address: string, city: string): Promise<[number, number] | null> => {
    try {
      const token = import.meta.env.VITE_MAPBOX_ACCESS_TOKEN || (window as any).VITE_MAPBOX_ACCESS_TOKEN || "pk.eyJ1IjoibWljaGFlbGVrIiwiYSI6ImNtYzBja3R5MzAwdDQya29kODdrNnNyYXQifQ._51Nw5m36McY40ID6SdWhQ";
  ...
  useEffect(() => {
    ...
    map.current = new mapboxgl.Map({
      ...
      center: getCityCenter(places.length > 0 ? places[0].city : 'lisbon'),
      zoom: 11,
    });
  ...
  useEffect(() => {
    ...
    const addMarkersToMap = async () => {
      ...
      for (const place of places.slice(0, 10)) { // Limit to 10 pins as requested
        ...
        const contactInfo = (place.phone || place.email) 
          ? `<div class="mb-2">
      ...
      if (validCoordinates && markers.current.length > 1) {
        ...
        const [lng, lat] = places[0].latitude && places[0].longitude 
          ? [parseFloat(places[0].longitude), parseFloat(places[0].latitude)]
    ...
    return (
      <div className="h-[500px] flex items-center justify-center bg-gray-100 rounded-lg">
        <div className="text-center">
          ...
          <p className="text-gray-600">Map functionality requires Mapbox configuration</p>
        </div>
      </div>
    );
  }
