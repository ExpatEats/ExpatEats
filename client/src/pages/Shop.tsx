...
import { MapPin, ShoppingCart, Leaf, Heart, Home, Shirt, Sparkles } from 'lucide-react';
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      <div className="text-center mb-8">
        ...
        <h1 className="font-montserrat text-3xl md:text-4xl font-bold mb-3">Shop Sustainably</h1>
        <p className="text-xl text-gray-600 max-w-2xl mx-auto">
          Find sustainable, organic, and local options in the Lisbon metropolitan area
        </p>
      </div>
      ...
      <Card className="mb-8">
        ...
        <CardContent>
          <p className="text-gray-600 mb-4">Choose one or multiple locations in the Lisbon area:</p>
              ...
              {selectedCategoryData.subcategories.map((subcategory) => (
                ...
                >
                  <div className="flex items-center gap-2 mb-2">
                    {subcategory.id === 'grocery-market' && <ShoppingCart className="h-4 w-4 text-[#E07A5F]" />}
                    {subcategory.id === 'supplements' && <Leaf className="h-4 w-4 text-[#E07A5F]" />}
                    {subcategory.id === 'beauty' && <Sparkles className="h-4 w-4 text-[#E07A5F]" />}
                    {subcategory.id === 'cleaning' && <Home className="h-4 w-4 text-[#E07A5F]" />}
                    {subcategory.id === 'housewares' && <Home className="h-4 w-4 text-[#E07A5F]" />}
                    {subcategory.id === 'clothing' && <Shirt className="h-4 w-4 text-[#E07A5F]" />}
                    {subcategory.id === 'wellness' && <Heart className="h-4 w-4 text-[#E07A5F]" />}
                    <h4 className="font-medium">{subcategory.name}</h4>
                  </div>
                  <p className="text-sm text-gray-600">
                    {subcategory.id === 'supplements' && 'Quality supplements including online resources that ship to Portugal'}
                    {subcategory.id === 'grocery-market' && 'Stores, markets, and private sellers with organic, local, sustainable options'}
                    {subcategory.id === 'beauty' && 'Organic and non-toxic beauty products'}
                    {subcategory.id === 'cleaning' && 'Non-toxic cleaning products and solutions'}
                    {subcategory.id === 'housewares' && 'Non-toxic furniture and filtered water systems'}
                    {subcategory.id === 'clothing' && 'Sustainable and organic clothing options'}
                    {subcategory.id === 'wellness' && 'Wellness services and products'}
                  </p>
                </div>
              ))}
            </div>
      ...
      <div className="text-center">
        <Button 
          ...
          disabled={selectedLocations.length === 0 || !selectedSubcategory}
          className="bg-[#E07A5F] hover:bg-[#E07A5F]/90 text-white px-8 py-3 text-lg font-medium"
        >
          Find {selectedSubcategory ? selectedCategoryData?.subcategories.find(s => s.id === selectedSubcategory)?.name : 'Options'}
        </Button>
