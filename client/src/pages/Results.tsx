...
const Results = () => {
  ...
  const saveMutation = useMutation({
    mutationFn: async ({ storeId, action }: { storeId: number; action: 'save' | 'unsave' }) => {
    ...
    onSuccess: (data, variables) => {
      toast({
        title: variables.action === 'save' ? 'Store Saved' : 'Store Removed',
        description: variables.action === 'save' 
          ? 'Added to your favorites' 
          : 'Removed from your favorites',
      });
      queryClient.invalidateQueries({ queryKey: ['/api/user/saved-stores'] });
    },
    onError: () => {
      toast({
        title: 'Error',
  ...
  const getCategoryTitle = () => {
    if (category && category.includes('Health Food Store')) return 'Supplements Guide';
  ...
  const getResultsDescription = () => {
    ...
    const isSupplements = category && category.includes('Health Food Store');
  ...
  return (
    <div className="w-full max-w-full px-4 py-8 overflow-x-hidden">
      ...
      <div className="mb-8">
        ...
        <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-2">Results</h1>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 mb-6">
            We couldn't find any businesses matching your criteria.<br />
                      ...
                      {(place.phone || place.email) && (
                        <div>
                          ...
                          <div className="space-y-1">
                            ...
                            {place.email && (
                              <p className="text-sm text-gray-600">✉️ {place.email}</p>
                            )}
                          </div>
                        </div>
                      )}
                      ...
                      {place.tags && place.tags.length > 0 && (
                        <div>
                          ...
                          <div className="flex flex-wrap gap-1">
                            {place.tags.slice(0, 4).map((tag: string, index: number) => (
                              <Badge 
                                key={index} 
                                variant="secondary" 
