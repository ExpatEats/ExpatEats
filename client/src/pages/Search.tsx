...
import { MapPin, Search as SearchIcon, Filter, ShoppingBag, Carrot, Store, Egg, Users, ArrowRight, ExternalLink } from 'lucide-react';
  ...
  const filterSources = () => {
    ...
    if (searchTerm) {
      ...
      results = results.filter(source => 
        ...
        (source.description && source.description.toLowerCase().includes(searchTermLower)) ||
        (source.address && source.address.toLowerCase().includes(searchTermLower)) ||
        (source.category && source.category.toLowerCase().includes(searchTermLower))
      );
    }
    ...
    if (selectedCategories.length > 0) {
      results = results.filter(source => {
        ...
        if (source.category && selectedCategories.includes(source.category)) {
          return true;
        }
        ...
        for (const selectedCategory of selectedCategories) {
          ...
          if (selectedCategory.includes('farm')) {
            ...
            const hasFarmTag = source.tags && source.tags.some(tag => 
              tag.includes('farm') || 
            ...
            const hasFarmCategory = source.category && 
          ...
          else if (selectedCategory === 'zero waste shop' || selectedCategory === 'bulk store') {
            const hasZeroWasteTag = source.tags && source.tags.some(tag => 
              tag.includes('zero-waste') || 
              tag.includes('bulk') ||
              tag.includes('low-waste')
            );
            
            const hasMatchingCategory = source.category && 
          ...
          // Health food store matching
            ...
            const hasHealthFoodTag = source.tags && source.tags.some(tag => 
              tag === 'organic' || 
              tag === 'gluten-free' ||
              tag === 'vegan' ||
              tag === 'dairy-free'
            ...
            if (hasHealthFoodTag && source.category && 
          ...
          // Market related matching
            ...
            const isMarketRelated = source.category && 
              (source.category.includes('market') || 
               source.category.includes('store') ||
      ...
      results = results.filter(source => {
        ...
        return savedPreferences.some(pref => {
          ...
          if (pref === 'bio/organic' && (
              ...
              source.category && source.category.toLowerCase().includes('organic')
          )) {
            return true;
          }
          
          // Handle special case for other known preferences that might have different naming
          if (pref === 'gluten-free' && source.tags.some(tag => 
              tag.includes('gluten-free') || tag.includes('gluten free')
          )) {
            return true;
          }
          
          if (pref === 'dairy-free' && source.tags.some(tag => 
              tag.includes('dairy-free') || tag.includes('dairy free') || tag.includes('lactose-free')
          )) {
            return true;
          }
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="max-w-4xl mx-auto">
        <div className="flex flex-col items-center mb-4 sm:flex-row sm:justify-between sm:items-center">
          <h1 className="font-montserrat text-3xl font-bold">Find Local Food Sources</h1>
        ...
        <div className="bg-white rounded-xl shadow-md p-4 mb-6">
          ...
          {showFilters && (
            <div className="mt-4 p-4 bg-gray-50 rounded-lg">
              <h3 className="font-semibold mb-3">Filter by Location Type</h3>
              ...
              <h3 className="font-semibold mb-3">Your Dietary Preferences</h3>
              <div className="flex flex-wrap gap-2 mb-4">
                ...
                {savedPreferences.length === 0 && (
                  <p className="text-gray-500 text-sm">No preferences saved. <Link to="/" className="text-primary underline">Set up your profile</Link></p>
                )}
              </div>
        ...
        <p className="text-gray-600 mb-6">
          {filteredSources.length} {filteredSources.length === 1 ? 'place' : 'places'} found in Lisbon
        </p>
          ...
          <div className="space-y-6">
            {filteredSources.length > 0 ? (
                  ...
                  <div className="flex flex-col md:flex-row">
                      ...
                      {(() => {
                        ...
                        switch(source.name) {
                          ...
                          default:
                            ...
                            if (source.category?.toLowerCase().includes('supermarket')) {
                              return (
                                <img 
                                  src="https://images.unsplash.com/photo-1534723452862-4c874018d66d?ixlib=rb-4.0.3&auto=format&fit=crop&w=600&q=80"
                                  alt={source.name}
                            ...
                            } else if (source.category?.toLowerCase().includes('health')) {
                              return (
                                <img 
                            ...
                            } else if (source.category?.toLowerCase().includes('bakery')) {
                              return (
                                <img 
                            ...
                            } else if (source.category?.toLowerCase().includes('market')) {
                              return (
                                <img 
                            ...
                            } else if (source.category?.toLowerCase().includes('bulk') || source.tags?.includes('zero-waste')) {
                              return (
                                <img 
                    ...
                    <CardContent className="md:w-2/3 p-6">
                      <div className="flex justify-between items-start">
                        <div>
                          <h2 className="text-xl font-bold font-montserrat mb-1">{source.name}</h2>
                          <div className="flex items-center text-gray-500 mb-3">
                            <Badge className="mr-2 flex items-center gap-1">
                              {getCategoryIcon(source.category)}
                              <span>{source.category ? source.category.split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ') : 'Food Source'}</span>
                            </Badge>
                        ...
                        {source.website && (
                          <a href={source.website} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline flex items-center gap-1">
                      ...
                      <div className="flex justify-between gap-2">
                        <Link to="/contact">
                          <Button variant="outline" className="flex items-center border-accent text-accent hover:bg-accent/5">
                            I want help shopping here <ArrowRight className="h-4 w-4 ml-2" />
                          </Button>
                        </Link>
                        <Link to="/contact">
                          <Button className="bg-accent text-white hover:bg-accent/90">
                            I want help meal planning <ArrowRight className="h-4 w-4 ml-2" />
                          </Button>
                        </Link>
                      </div>
                    </CardContent>
              ...
              <div className="text-center py-10 bg-gray-50 rounded-lg">
                <p className="text-gray-600 mb-4">No food sources match your criteria.</p>
