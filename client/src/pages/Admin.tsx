...
import { Users, Mail, MapPin, User, LogOut, Clock, CheckCircle, XCircle } from 'lucide-react';
...
export default function Admin() {
  ...
  const approvePlaceMutation = useMutation({
    mutationFn: async ({ placeId, adminNotes }: { placeId: number; adminNotes?: string }) => {
      return await apiRequest('POST', `/api/admin/approve-place/${placeId}`, { adminNotes });
    },
    onSuccess: () => {
      toast({
        title: 'Success',
  ...
  const rejectPlaceMutation = useMutation({
    mutationFn: async ({ placeId, adminNotes }: { placeId: number; adminNotes: string }) => {
      return await apiRequest('POST', `/api/admin/reject-place/${placeId}`, { adminNotes });
    },
    onSuccess: () => {
      toast({
        title: 'Success',
    ...
    <div className="container py-8">
      ...
      <Card className="w-full max-w-6xl mx-auto mb-8">
        <CardHeader>
          ...
          <CardDescription>
            Review and approve user-submitted locations before they appear publicly
          </CardDescription>
        </CardHeader>
        <CardContent>
          {pendingPlaces.length === 0 ? (
              ...
              {pendingPlaces.map((place: any) => (
                <div key={place.id} className="border border-gray-200 rounded-lg p-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                      <p className="text-sm"><strong>Description:</strong> {place.description}</p>
                      <p className="text-sm mt-2"><strong>Address:</strong> {place.address}</p>
                      <p className="text-sm"><strong>Location:</strong> {place.city}, {place.country}</p>
                    </div>
                    <div>
                      ...
                      {place.website && (
                        <p className="text-sm mt-2"><strong>Website:</strong> {place.website}</p>
                      )}
                      <p className="text-sm mt-2"><strong>Submitted:</strong> {new Date(place.createdAt).toLocaleDateString()}</p>
                    </div>
                  </div>
                  
                  <div className="flex gap-2 pt-3 border-t">
                    <Button
                      onClick={() => approvePlaceMutation.mutate({ placeId: place.id })}
                      disabled={approvePlaceMutation.isPending || rejectPlaceMutation.isPending}
                    ...
                    <Button
                      onClick={() => {
                        ...
                        if (reason) {
                          rejectPlaceMutation.mutate({ placeId: place.id, adminNotes: reason });
                        }
                      }}
                      disabled={approvePlaceMutation.isPending || rejectPlaceMutation.isPending}
                      variant="destructive"
              ...
              {users.map((user) => (
                <div key={user.id} className="border border-gray-200 rounded-lg p-4">
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    {user.bio && (
                      <div className="pt-2">
                        <span className="text-xs text-gray-500 line-clamp-2">{user.bio}</span>
                      </div>
                    )}
                  </div>
                </div>
      ...
      <Card className="w-full max-w-3xl mx-auto">
        <CardHeader>
          ...
          <CardDescription>
            Enter details about a local food source that might be helpful for other expats
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="Market name or business name" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      >
                        ...
                        <SelectContent>
                          <SelectItem value="grocer">Grocery Store</SelectItem>
                          <SelectItem value="market">Farmers Market</SelectItem>
                          <SelectItem value="farm">Farm</SelectItem>
                          <SelectItem value="zerowaste">Zero Waste Shop</SelectItem>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="Link to an image of this place" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <div className="space-y-3">
                <FormLabel>Dietary Preferences Available</FormLabel>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {dietaryPreferences.map((preference) => (
                    <div key={preference.id} className="flex items-center space-x-2">
                      <Checkbox 
                        id={preference.id} 
                        checked={selectedTags.includes(preference.id)}
                        onCheckedChange={() => handleTagToggle(preference.id)}
                      />
                      <label
              ...
              >
                {addFoodSourceMutation.isPending ? "Adding..." : "Add Food Source"}
              </Button>
            </form>
          </Form>
        </CardContent>
